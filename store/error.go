...
func NewIndexFormatTooNewError(in *DataInput, version, minVersion, maxVersion int) error {
	return errors.New(fmt.Sprintf(
		"Format version is not supported (resource: %v): %v (needs to be between %v and %v)",
		in, version, minVersion, maxVersion))
}

func NewIndexFormatTooOldError(in *DataInput, version, minVersion, maxVersion int) error {
	return errors.New(fmt.Sprintf(
