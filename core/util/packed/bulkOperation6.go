...
func (op *BulkOperationPacked6) decodeByteToInt(blocks []byte, values []int32, iterations int) {
	...
	for i := 0; i < iterations; i ++ {
		...
		blocksOffset++
		values[valuesOffset] = int32( byte(uint8(byte0) >> 2))
		valuesOffset++
		byte1 := blocks[blocksOffset]
		blocksOffset++
		values[valuesOffset] = int32(((byte0 & 3) << 4) | byte(uint8(byte1) >> 4))
		valuesOffset++
		byte2 := blocks[blocksOffset]
		blocksOffset++
		values[valuesOffset] = int32(((byte1 & 15) << 2) | byte(uint8(byte2) >> 6))
		valuesOffset++
